...
| 18 | **Nested constructs synthesise their artificial links once per enclosing instance, and the duplicates reach the PGT.** `lgn_to_pgn` appends the loop-circle and group-start links [lg.py:303](dlg/dropmake/lg.py#L303), [:316](dlg/dropmake/lg.py#L316) on every visit to a construct, and visits a nested construct once per instance of each enclosing construct [lg.py:349](dlg/dropmake/lg.py#L349). `add_input`/`add_output` deduplicate; `self._lg_links.append` does not, and neither does the link loop — each copy re-wires the same DROP pairs, so the PGT carries repeated `consumers`/`inputs`/`ports` entries. Corpus, 2026-09-24: 257 of 338 synthesised links are duplicates, across 17 graphs; deduplicating them changes all 17 PGTs | **not fixed in P4-2** — its gate is byte-for-byte, so the `synthesise_links` pre-pass keeps the per-instance walk and says why. Fixing it is a 17-graph corpus change and needs a decision and its own issue; none filed yet. Whichever of the fix and `synthesise_links` moving onto handlers lands second must not re-introduce the repeat. Migration map §7 **B9** | none |
...
| 20 | **The Gather drain splices every input onto the first output DROP only** (`v[2][0]` [lg.py:766](dlg/dropmake/lg.py#L766)), and takes `is_stream` from whichever link created the cache entry (`v[-1]` [lg.py:769](dlg/dropmake/lg.py#L769)). Suspected, not observed: no corpus Gather has more than one output DROP, and switching `is_stream` to the input link changes nothing | kept in P4-2 for byte parity; the drain now lives in `wire()` as a local. Decide in P4-3's Gather PR. Migration map §7 **B11**, **B12** | none |
| 21 | **Gather sequentialisation could never run** — `getPortName(port=...)` [lg.py:595](dlg/dropmake/lg.py#L595) is not a valid keyword, so any graph reaching the branch raised `TypeError` | **fixed in P4-2** (a sanctioned behaviour change on graphs that crashed before; no corpus graph reaches it). The chained edges still bypass `_link_drops`, and inputs are re-used when there are fewer than `gather_width` — for P4-3. Migration map §7 **B13** | none |
