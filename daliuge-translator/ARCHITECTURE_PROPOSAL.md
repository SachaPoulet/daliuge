...
Scope section forbids regardless of how tempting the 1247-line module makes them — and **row
15**, which is a genuine bug but not one the restructure touches. Row 15 is recorded here so
it is not rediscovered, and as migration map §7 **B6**; it has deliberately **no issue in
[ARCHITECTURE_ISSUE_PLAN.md](ARCHITECTURE_ISSUE_PLAN.md)**, because every phase there is
either a move or a sanctioned behaviour change and this is neither.
