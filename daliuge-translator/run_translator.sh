...
case "$1" in
    ...
    "dev")
        ...
        docker run -d -h dlg-tm --volume "$PWD/dlg/dropmake:/dlg/lib/python3.10/site-packages/dlg/dropmake" --name daliuge-translator --rm -t -p 8084:8084 icrar/daliuge-translator:${C_TAG}
        sleep 3
        docker exec -u root daliuge-translator bash -c "service avahi-daemon stop > /dev/null 2>&1 && service dbus restart > /dev/null 2>&1 && service avahi-daemon start > /dev/null 2>&1";;
    "slim")
        echo "Running Translator development version in background..."
        docker run -d -h dlg-tm --volume "$PWD/dlg/dropmake:/dlg/lib/python3.10/site-packages/dlg/dropmake" --name daliuge-translator --rm -t -p 8084:8084 icrar/daliuge-translator.slim:${VCS_TAG}
    ...
    "casa")
        ...
        docker run -h dlg-tm --volume "$PWD/dlg/dropmake:/dlg/lib/python3.10/site-packages/dlg/dropmake" --name daliuge-translator --rm -t -p 8084:8084 icrar/daliuge-translator:${VCS_TAG}
        sleep 3
        docker exec -u root daliuge-translator bash -c "service avahi-daemon stop > /dev/null 2>&1 && service dbus restart > /dev/null 2>&1 && service avahi-daemon start > /dev/null 2>&1";;
    *)
