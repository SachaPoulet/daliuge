...
def _git(repository: Path, *args: str) -> str:
    import subprocess
        ...
        ["git", "-c", "user.email=corpus@test", "-c", "user.name=corpus",
         "-C", str(repository), *args],
        capture_output=True, text=True, check=True)
    return done.stdout.strip()


@pytest.fixture(name="baseline_repo")
def _baseline_repo(tmp_path, monkeypatch):
    """A throwaway repo standing in for a DALiuGE checkout, pinned at its first commit.

    Hermetic on purpose: asserting against the real tree would pass on this branch and
    fail on any branch that has legitimately changed the translator.
    """
    repository = tmp_path / "daliuge"
    source = repository / "daliuge-translator" / "dlg" / "dropmake"
    source.mkdir(parents=True)
    (source / "pg_generator.py").write_text("# baseline\n")
    _git(repository.parent, "init", "-q", str(repository))
    _git(repository, "add", "-A")
    _git(repository, "commit", "-qm", "baseline")

    pin = _git(repository, "rev-parse", "HEAD")
    monkeypatch.setattr(provenance, "PINS", {"daliuge_baseline_commit": pin})
    monkeypatch.setattr(provenance, "translator_source",
                        lambda cli: (source / "pg_generator.py").resolve())
    return repository


def test_baseline_accepts_a_clean_checkout_at_the_pin(baseline_repo):
    assert provenance.assert_baseline(cli="dlg") == baseline_repo.resolve()


def test_baseline_refuses_a_committed_translator_change(baseline_repo):
    """The case that matters: goldens generated by a build that is not the baseline."""
    (baseline_repo / "daliuge-translator/dlg/dropmake/pg_generator.py").write_text("# no\n")
    _git(baseline_repo, "commit", "-qam", "change the translator")

    with pytest.raises(provenance.ProvenanceError) as raised:
        provenance.assert_baseline(cli="dlg")
    assert "carries translator changes" in str(raised.value)
    assert "pg_generator.py" in str(raised.value)


def test_baseline_refuses_an_uncommitted_translator_change(baseline_repo):
    """HEAD says nothing about the working tree, and an editable install runs the tree."""
    (baseline_repo / "daliuge-translator/dlg/dropmake/pg_generator.py").write_text("# no\n")
...
def test_baseline_refuses_when_legacy_repo_is_not_where_the_cli_lives(baseline_repo,
                                                                     tmp_path):
    with pytest.raises(provenance.ProvenanceError) as raised:
        provenance.assert_baseline(cli="dlg", legacy_repo=tmp_path / "elsewhere")
    message = str(raised.value)
    assert "--legacy-repo does not hold the translator" in message
    # Both sides named, so the mismatch is readable rather than just asserted.
    assert str(baseline_repo.resolve()) in message
    assert str((tmp_path / "elsewhere").resolve()) in message


def test_baseline_refuses_a_translator_outside_any_repository(tmp_path, monkeypatch):
    """A non-editable install has no history, so its provenance is unknowable."""
    loose = tmp_path / "site-packages" / "dlg" / "dropmake"
    loose.mkdir(parents=True)
    (loose / "pg_generator.py").write_text("# copied\n")
    monkeypatch.setattr(provenance, "translator_source",
                        lambda cli: (loose / "pg_generator.py").resolve())
...
def test_interpreter_is_read_from_the_console_script_shebang(tmp_path):
    script = tmp_path / "dlg"
    script.write_text("#!/opt/legacy/bin/python\nimport sys\n")
    assert provenance._interpreter_for(script) == Path("/opt/legacy/bin/python")


def test_a_script_without_a_shebang_is_refused(tmp_path):
    script = tmp_path / "dlg"
    script.write_text("not a script\n")
    with pytest.raises(provenance.ProvenanceError):
        provenance._interpreter_for(script)


def test_legacy_repo_is_taken_out_of_the_argument_list():
    assert golden._take_legacy_repo(["mpi_simple", "--legacy-repo", "/tmp/x"]) == (
        ["mpi_simple"], Path("/tmp/x"))


def test_argument_list_without_legacy_repo_is_untouched():
    assert golden._take_legacy_repo(["mpi_simple"]) == (["mpi_simple"], None)


def test_legacy_repo_without_a_path_is_refused():
    with pytest.raises(SystemExit):
        golden._take_legacy_repo(["--legacy-repo"])
